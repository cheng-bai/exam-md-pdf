...
3. 已知集合 $A = \{ x \mid  x <  - 1$ 或 $x > 4\} , B = \{ x \mid  \left| {x + 1}\right|  < 1, x \in  \mathbf{R}\}$ ，则 $A \cap  B =$ ___.

4. 已知 $\mathrm{i}$ 是虚数单位，则 $\frac{5\mathrm{i}}{2 - \mathrm{i}} =$ ___.

5. 若 ${\left( x + a\right) }^{6}$ 的展开式中 ${x}^{3}$ 的系数为 160，则实数 $a =$ ___.
...
9. 已知角 $\alpha$ 的顶点与坐标原点重合,始边与 $x$ 轴的正半轴重合,终边与以坐标原点为圆心、1 为半径的圆交于点 $P$ ,若点 $P$ 的横坐标与纵坐标之和为 $\frac{1}{2}$ ,则 $\tan \alpha  + \cot \alpha$ 的值为___.
...
![bo_d4nusgref24c73bbkl2g_0_1040_1491_279_263_0.jpg](images/bo_d4nusgref24c73bbkl2g_0_1040_1491_279_263_0.jpg)

(第11题图)

12. 若图形 $\Gamma$ 上的每个点都在圆 $C$ 上或在圆 $C$ 内部,则称圆 $C$ 为 $\Gamma$ 的一个覆盖圆. 设 $\Gamma$ 由函数 $y = 2\left| x\right| \; - 4\left( {-2 < x < 2}\right)$ 与 $y = \sqrt{2\left| x\right|  - {x}^{2}}$ 的图像所组成，则 $\Gamma$ 的最小覆盖圆的半径为___.
...
14. 设 $\alpha ,\beta$ 是两个不同的平面, $l$ 是一条直线,以下命题正确的是 ( ).
...
15. 若数列 $\left\{  {a}_{n}\right\}$ 同时满足如下条件:(i) $\left\{  {a}_{n}\right\}$ 是无穷数列；(ii) $\left\{  {a}_{n}\right\}$ 是严格增数列；(iii)存在正数 $M$ ， 使得对任意的 $n \in  {\mathbf{N}}^{ * }$ ，都有 $\left\{  {a}_{n}\right\}$ 的前 $n$ 项的和 ${S}_{n} \in  \left\lbrack  {-M, M}\right\rbrack$ ，则称 $\left\{  {a}_{n}\right\}$ 具有性质 $P$ . 关于如下结论:①存在等差数列具有性质 $P$ ；②存在等比数列具有性质 $P$ . 其中正确的说法是 ( ).

A. ①和②均证确 B. ①和②均错误

C. ①正确，②错误 D. ①错误，②正确

16. 已知点 $P\left( {-5,0}\right)$ ， ${Q}_{1}$ ， ${Q}_{2} \in  \{ \left( {x, y}\right) \parallel x \mid   \leq  1$ ， $y \mid   \leq  1$ )，则 $\overrightarrow{P{Q}_{1}} \cdot  \overrightarrow{P{Q}_{2}}$ 的取值范围是 ( ).
...
## 17. (本题满分 14 分) 本题共有 3 个小题, 第 1、2 小题满分各为 4 分, 第 3 小题满分 6 分.

一家水果店的店长为了解本店苹果的日销售情况, 记录了过去 20 天苹果的日销售量 (单位: kg), 按从小到大的排序结果如下:62,74,75,84,84,85,85,85,86,87,89,92,93,94,97,99,101,104,107,117.
...
如图，在几何体 ${ABCDE}$ 中，四边形 ${ABCD}$ 是矩形， ${AB}$ 上平面 ${BEC}$ ， ${BE} \bot  {EC}$ ， ${AB} = {BE} = {EC} = 2$ ，
...
![bo_d4nusgref24c73bbkl2g_2_1035_500_297_249_0.jpg](images/bo_d4nusgref24c73bbkl2g_2_1035_500_297_249_0.jpg)

(第 18 题图)

19. (本题满分 14 分) 本题共有 2 个小题, 第 1 小题 6 分, 第 2 小题 8 分.

在 $\bigtriangleup {ABC}$ 中， $\angle {BAC} = \frac{3\pi }{4},{AB} = 6,{AC} = 3\sqrt{2}$ .
...
![bo_d4nusgref24c73bbkl2g_2_903_1374_429_175_0.jpg](images/bo_d4nusgref24c73bbkl2g_2_903_1374_429_175_0.jpg)

(第 19 题图)

20. (本题满分 18 分)本题共有 3 个小题, 第 1 小题满分 4 分, 第 2 小题满分 6 分, 第 3 小题满分 8 分.

已知双曲线 $\Gamma$ 的中心位于坐标原点 $O$ ,焦点 ${F}_{2},{F}_{1}$ 分别在 $x$ 轴的正、负半轴上, $\left| {{F}_{1}{F}_{2}}\right|  = 2\sqrt{2}$ ,直线 $y =  - x$ 是 $\Gamma$ 的一条渐近线,直线 ${l}_{1} : y = \sqrt{2}x + t\left( {t < 0}\right)$ 与 $\Gamma$ 有且只有一个公共点 $P$ .
...
![bo_d4nusgref24c73bbkl2g_3_1047_745_292_274_0.jpg](images/bo_d4nusgref24c73bbkl2g_3_1047_745_292_274_0.jpg)

(第 20 题图)

## 21. (本题满分 18 分) 本题共有 3 个小题, 第 1 小题满分 4 分, 第 2 小题满分 6 分, 第 3 小题满分 8 分.

已知函数 $y = f\left( x\right)$ 的定义域为 $D$ ,对于给定实数 $t$ ,定义集合 ${V}_{f\left( t\right) } = \{ x \mid  f\left( {x + t}\right)  \geq  f\left( x\right) \}$ .
