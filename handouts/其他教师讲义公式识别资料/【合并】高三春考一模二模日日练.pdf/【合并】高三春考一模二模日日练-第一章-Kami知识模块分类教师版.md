# 高三春考一模中档题日日练（第一章·知识模块分类教师版）

> chengbai 数学讲义｜按主要解题入口分类；保留原日日练来源、答案、解析与待核标注。
...
**解析：** 令 $x=a-1\;(0<x<3)$，原式为 $\dfrac{x+1}{3-x}+\dfrac1x$。求导或通分比较可得在 $x=1$，即 $a=2$ 时取最小值 $2$。
...
**解析：** 由 $a_3^2=a_6$ 得公比 $q=1/3$，故无穷和为 $(1/3)/(1-1/3)=1/2$。
