...
![0_541_1706_317_281_0.jpg](images/0_541_1706_317_281_0.jpg)
...
【解析】对于 $\mathrm{A}$ ,当 $x < 0$ 时, $y = x + \frac{4}{x} < 0$ ,故 $\mathrm{A}$ 错误;
...
【解析】解: 对于 $A$ ,由 ${\left( a - b\right) }^{2} \geq  0$ ,知 ${a}^{2} + {b}^{2} \geq  {2ab}$ ,即 $A$ 错误;
...
【解析】解: $\because a > 0, b > 0,\therefore 1 = {2a} + b \geq  2\sqrt{2ab}$ ,
...
【解析】解: $\because {4x} + \frac{1}{y} = 1, x, y \in  {R}^{ + },\therefore 4{x}^{2} + \frac{x}{y} = x$ ,即 $\frac{x}{y} =  - 4{x}^{2} + x =  - 4{\left( x - \frac{1}{8}\right) }^{2} + \frac{1}{16} \leq  \frac{1}{16}$ ,
...
【解析】解: $x > 1$ ,则 $x + \frac{2}{x - 1} = x - 1 + \frac{2}{x - 1} + 1 \geq  2\sqrt{2} + 1$ ,当且仅当 $x - 1 = \frac{2}{x - 1}$ ,即 $x = 1 + \sqrt{2}$ 时等号成立, 故答案为: $1 + \sqrt{2}$ .
...
【解析】解: $y =  - x + \frac{1}{4 - x} = 4 - x + \frac{1}{4 - x} - 4 =  - \left\lbrack  {\left( {x - 4}\right)  + \frac{1}{x - 4}}\right\rbrack   - 4 \leq   - 2\sqrt{\left( {x - 4}\right)  \times  \frac{1}{x - 4}} - 4 =  - 6$ , 即函数有最大值-6,故选: $A$ .
...
【解析】 $x\sqrt{1 + {y}^{2}} = \sqrt{{x}^{2}\left( {1 + {y}^{2}}\right) } \leq  \frac{{x}^{2} + 1 + {y}^{2}}{2} = \frac{5}{2}$ ,当且仅当 ${x}^{2} = 1 + {y}^{2}$ ,即 $x = \frac{\sqrt{10}}{2}, y = \frac{\sqrt{6}}{2}$ 时,等号成立. $\therefore$ 所求最大值为 $\frac{5}{2}$ . 故选: B.
...
【解析】解: 由已知: $\frac{1}{x} + \frac{1}{y} = \left( {\frac{1}{x} + \frac{1}{y}}\right) \left( {x + {2y}}\right)  = 1 + \frac{2y}{x} + \frac{x}{y} + 2 \geq  3 + 2\sqrt{2}$ ,
...
【解析】解: $\because$ 正数 $m, n$ 满足 $m\left( {n - 1}\right)  = {8n},\therefore \frac{8}{m} + \frac{1}{n} = 1$ .
...
【解析】解: $\because x > 0, y > 0$ ,且 $\frac{1}{x + 1} + \frac{1}{y} = \frac{1}{2}$ ,
...
【解析】解: 因为 $a\text{ 、 }b$ 均为正实数,且 $a + \frac{2}{b} = 3$ ,则
...
【解析】方法一: 令 ${2x} + y = t\left( {t > 0}\right)$ ,则 $y = t - {2x},\therefore \frac{2y}{x} + \frac{9x}{{2x} + y} = \frac{{2t} - {4x}}{x} + \frac{9x}{t} = \frac{2t}{x} + \frac{9x}{t} - 4 \geq  6\sqrt{2} - 4$ , 当且仅当 $\frac{2t}{x} = \frac{9x}{t}$ ,即 $t = \frac{3\sqrt{2}}{2}x$ 时等号成立.
...
【解析】解: 直线 $l : \frac{2x}{{2b} + a} + \frac{y}{a + b} = 1$ 经过第一象限内的点 $P\left( {\frac{1}{a},\frac{1}{b}}\right)$ ,
...
【解析】因为 $\mathrm{a} > \mathrm{b} > \mathrm{c}$ ,所以 $\mathrm{a} - \mathrm{b} > 0,\mathrm{\;b} - \mathrm{c} > 0,\mathrm{\;a} - \mathrm{c} > 0$ .
...
【解析】解: 由 $x, y \in  {\mathbb{R}}^{ + },{xy} = x + y + 1 \geq  2\sqrt{xy} + 1 \Leftrightarrow  {\left( \sqrt{xy}\right) }^{2} - 2\sqrt{xy} - 1 \geq  0$ ,
...
【解析】解: $a > 0, b > 0,\therefore \sqrt{ab} = \frac{2}{a} + \frac{3}{b} \geq  \frac{2\sqrt{6}}{\sqrt{ab}}$ ,即 $\sqrt{ab} \geq  \frac{2\sqrt{6}}{\sqrt{ab}}$ ,
...
【解析】解: $\because {x}^{2}{y}^{2} + {x}^{2} + {y}^{2} = 8,\therefore 8 = {x}^{2}{y}^{2} + {x}^{2} + {y}^{2} \leq  {\left( \frac{{x}^{2} + {y}^{2}}{2}\right) }^{2} + {x}^{2} + {y}^{2}$ ,
...
【解析】解: $\because a > 0, b > 0;\therefore a + {4b} \geq  4\sqrt{ab}$ ,当 $a = {4b}$ 时取 “ $=$ ”; $\therefore {\left( a + 4b\right) }^{2} \geq  {16ab}$ ;
...
【解析】由题得, $\left( {x - a}\right) \left( {x - {3a}}\right)  < 0\left( {a < 0}\right)$ 的解集为 $\left\{  {x \mid  {x}_{1} < x < {x}_{2}}\right\}$ ,则 ${x}_{1} = {3a},{x}_{2} = a$ ,函数 $y = {x}_{1} + \frac{a}{{x}_{1}{x}_{2}} = {3a} + \frac{a}{{3a} \cdot  a}$ ,又 $a < 0$ ,则 $3\left( {-a}\right)  + \frac{1}{3\left( {-a}\right) } \geq  2\sqrt{3\left( {-a}\right)  \cdot  \frac{1}{3\left( {-a}\right) }} = 2$ ,故 $y = {3a} + \frac{1}{3a} \leq   - 2$ , 当且仅当 ${3a} = \frac{1}{3a}$ ，即 $a =  - \frac{1}{3}$ 时，取得等号，函数有最大值 -2 . 故选: $C$
...
【解析】解: 因为函数 $f\left( x\right)  = {2x} + \frac{1}{x} - 1, x < 0$ ;
...
【解析】因为 $\ln m + \ln n = \ln 2$ ,所以 ${mn} = 2, m > 0, n > 0$ ,
...
【解析】: $a - {2b} + 8 = 0$ ,则 ${2}^{a} + \frac{1}{{4}^{b}} \geq  2\sqrt{{2}^{a} \cdot  \frac{1}{{4}^{b}}} = 2\sqrt{{2}^{a - {2b}}} = 2\sqrt{{2}^{-8}} = \frac{1}{8}$ .
...
【解析】解: $\because$ 正数 $x\text{ 、 }y$ 满足: ${2x} + y - {xy} = 0,\therefore \frac{2}{y} + \frac{1}{x} = 1$ .
...
【解析】因为 $a + {3b} = 1$ ,所以 ${3a} + {9b} = 3$ ,即 $\left( {a + b}\right)  + \left( {{2a} + {8b}}\right)  = 3$ , $\therefore \frac{1}{a + b} + \frac{9}{2\left( {a + {4b}}\right) } = \left\lbrack  {\left( {a + b}\right)  + \left( {{2a} + {8b}}\right) }\right\rbrack  \left( {\frac{1}{a + b} + \frac{9}{2\left( {a + {4b}}\right) }}\right)  \times  \frac{1}{3} \; = \left\lbrack  {{10} + \frac{{2a} + {8b}}{a + b} + \frac{9\left( {a + b}\right) }{2\left( {a + {4b}}\right) }}\right\rbrack   \times  \frac{1}{3} \geq  \frac{1}{3} \times  \left( {{10} + 2\sqrt{9}}\right)  = \frac{16}{3}$ ,
...
【解析】解: 设 $x + 2 = s, y + 4 = t$ ,则 $s + t = x + y + 6 = 7$ ,即 $s + t = 7$ ,且 $x + y = 1$ .
...
【解析】 $\left( {{2x} + y}\right) \left( {\frac{m}{x} + \frac{2}{y}}\right)  = {2m} + 2 + \frac{4x}{y} + \frac{my}{x} \geq  {2m} + 2 + 4\sqrt{m}$ ,
...
【答案】 $\left( {-\infty , - 1}\right)  \cup  \left( {4, + \infty }\right)$

【解析】解: 正实数 $x, y$ 满足 $\frac{1}{x} + \frac{4}{y} = 1$ ,则 $x + \frac{y}{4} = \left( {\frac{1}{x} + \frac{4}{y}}\right) \left( {x + \frac{y}{4}}\right)  = 2 + \frac{4x}{y} + \frac{y}{4x} \geq  2 + 2\sqrt{\frac{4x}{y} \cdot  \frac{y}{4x}} = 4$ ,
...
【解析】由 $1 - 4{x}^{2} \geq  0$ 得 $- \frac{1}{2} \leq  x \leq  \frac{1}{2}$ ; 又 $x > 0$ ,所以 $0 < x \leq  \frac{1}{2}$
...
【解析】由题意可知, $f\left( x\right)  = \frac{\frac{4}{3}}{x} + \frac{3}{1 - x} = \frac{{\left( \frac{2\sqrt{3}}{3}\right) }^{2}}{x} + \frac{{\left( \sqrt{3}\right) }^{2}}{1 - x} \geq  \frac{{\left( \frac{2\sqrt{3}}{3} + \sqrt{3}\right) }^{2}}{1} = \frac{25}{3}$ ,
...
【解析】 $\because \sqrt{3}$ 是 ${3}^{a}$ 与 ${3}^{b}$ 的等比中项, $\therefore {3}^{a + b} = 3, a + b = 1$ ,
...
【解析】设等比数列 $\left\{  {a}_{n}\right\}$ 的公比为 $q\left( {q > 0}\right)$
...
【解析】由已知可得定点 $\mathrm{A}\left( {-2, - 1}\right)$ ,代入直线方程可得 ${2m} + n = 1$ ,从而
...
【答案】(1) $L\left( x\right)  = \left\{  \begin{array}{l}  - \frac{1}{3}{x}^{2} + {40x} - {200},0 < x < {80} \\  {1250} - \left( {x + \frac{10000}{x}}\right) , x \geq  {80} \end{array}\right.$ (2) 100 千件

【解析】解 (1) 因为每件商品售价为 0.05 万元,则 $x$ 千件商品销售额为 ${0.05} \times  {1000x}$ 万元,依题意得:
...
![12_140_195_241_313_0.jpg](images/12_140_195_241_313_0.jpg)

【难度】

【答案】见解析

【解析】解: (1) 依题意广告牌的高为 ${tm}$ ,则 $\left( {x - 1}\right) \left( {t - {1.25}}\right)  = {45}$ ,
...
【解析】(1) 证明: $\frac{a}{\sqrt{b}} + \frac{b}{\sqrt{a}} + \sqrt{a} + \sqrt{b} = \frac{a}{\sqrt{b}} + \sqrt{b} + \frac{b}{\sqrt{a}} + \sqrt{a} \geq  2\sqrt{a} + 2\sqrt{b}$ ,当且仅当 $a = b$ 时等号成立, $\therefore \frac{a}{\sqrt{b}} + \frac{b}{\sqrt{a}} \geq  \sqrt{a} + \sqrt{b}$ .
...
【解析】解: $\because$ 点 $A\left( {-2, - 1}\right)$ 在直线 ${mx} + {ny} + 1 = 0$ 上, $\therefore  - {2m} - n + 1 = 0$ ,即 ${2m} + n = 1$ , $\because \frac{1}{m} + \frac{2}{n} = \left( {\frac{1}{m} + \frac{2}{n}}\right) \left( {{2m} + n}\right)  = 4 + \frac{n}{m} + \frac{4m}{n} \geq  4 + 2\sqrt{\frac{n}{m} \cdot  \frac{4m}{n}} = 8$ ,当且仅当 $\frac{n}{m} = \frac{4m}{n}$ ,即 $n = {2m}$ 时取等号, $\therefore \frac{1}{m} + \frac{2}{n}$ 的最小值为 8,
...
【解析】由题意 $h = m \cdot  {2}^{t} + {2}^{1 - t} \geq  2$ 恒成立, $m \geq  \frac{2}{{2}^{t}} - \frac{{2}^{1 - t}}{{2}^{t}}$ ,设 $x = \frac{1}{{2}^{t}}$ ,由 $t \geq  0$ 得 $0 < x \leq  1$ , $m \geq  {2x} - 2{x}^{2} =  - 2{\left( x - \frac{1}{2}\right) }^{2} + \frac{1}{2},0 < x \leq  1$ 时, $- 2{\left( x - \frac{1}{2}\right) }^{2} + \frac{1}{2}$ 的最大值为 $\frac{1}{2},\therefore m \geq  \frac{1}{2}$ . 故选: C.
...
【解析】 $\because \overrightarrow{a} = \left( {\frac{1}{2},{a}^{2} + 1}\right) ,\overrightarrow{b} = \left( {{2a},\frac{1}{a}}\right) ,\therefore \overrightarrow{a} \cdot  \overrightarrow{b} = \frac{1}{2} \times  \left( {2a}\right)  + \left( {{a}^{2} + 1}\right)  \times  \frac{1}{a} = {2a} + \frac{1}{a}$ ,
...
【答案】(1)400 吨；(2)每月不能获利，需要国家至少补贴35000 元才能不亏损

【解析】(1) 当每月处理量为 $x$ 吨时, $x \in  \left\lbrack  {{200},{500}}\right\rbrack$ ,每吨的平均处理成本为
...
【解析】证明: ${a}^{2} + {b}^{2} + {c}^{2} = \frac{1}{2}\left( {{a}^{2} + {b}^{2} + {c}^{2} + {a}^{2} + {b}^{2} + {c}^{2}}\right)  \geq  \frac{1}{2}\left( {{2ab} + {2ca} + {2bc}}\right)  = {ab} + {bc} + {ca}$ . $\therefore {a}^{2} + {b}^{2} + {c}^{2} \geq  {ab} + {bc} + {ca}$ .
...
【解析】解: 因为 $x{y}^{2} = 1,{y}^{2} \geq  0$ ,所以 $x > 0,{y}^{2} > 0$ ,则 ${4x} + {y}^{2} \geq  2\sqrt{{4x}{y}^{2}} = 4$ ,
...
【解析】 $\left( {\frac{x}{y} + y}\right) \left( {\frac{y}{x} + x}\right)  = 1 + \frac{{x}^{2}}{y} + \frac{{y}^{2}}{x} + {xy} = 2 + \frac{{x}^{3} + {y}^{3}}{xy} = 2 + {x}^{3} + {y}^{3} \geq  2 + 2\sqrt{{x}^{3}{y}^{3}} = 4$ .
...
【解析】由于 $a\text{ 、 }{3b}$ 的等差中项为 1,则 $a + {3b} = 2$ ,
...
【解析】解: $\because$ 正实数 $a, b$ 满足 $a + b = 4,\therefore a + 1 > 1, b + 3 > 3, a + 1 + b + 3 = 8$ ,
...
【解析】解: 直角三角形的两直角边为 $a\text{ 、 }b$ ,斜边为 $c$ ,面积为 $s$ ,周长 $L = 2$ ,
...
【解析】解: 因为正实数 $x, y$ 满足 $x + {2y} = {2xy}$ 且不等式 $\left( {x + {2y} - a}\right) {xy} + 1 \geq  0$ 恒成立,所以 $\left( {{2xy} - a}\right) {xy} + 1 \geq  0$ 恒成立,即 $a \leq  {2xy} + \frac{1}{xy}$ 恒成立,则 $a \leq  {\left( 2xy + \frac{1}{xy}\right) }_{\min }$ ,因为 ${2xy} + \frac{1}{xy} \geq  2\sqrt{2}$ ,
...
【解析】解: \\textcircled{1}因为 $t = \sqrt{4 + {x}^{2}} \geq  2$ ,所以 $\sqrt{{x}^{2} + 4} + \frac{1}{\sqrt{{x}^{2} + 4}} = t + \frac{1}{t}$ 在 $\lbrack 2, + \infty )$ 上单调递增,
...
【解析】 $f\left( x\right)  = \frac{{x}^{2} - {4x} + 5}{{2x} - 4} = \frac{1}{2} \times  \frac{{\left( x - 2\right) }^{2} + 1}{x - 2} = \frac{1}{2}\left\lbrack  {\left( {x - 2}\right)  + \frac{1}{x - 2}}\right\rbrack   \geq  \frac{1}{2} \times  2\sqrt{1} = 1$ ,
...
【解析】由 ${n}^{2}x + \frac{x}{x - {2018}} > {2019}{n}^{2}$ 整理得, ${n}^{2}\left( {x - {2018}}\right)  + \frac{2018}{x - {2018}} > {n}^{2} - 1$ ,
...
【答案】(1)200吨；(2) $\left\lbrack  {{100},{300}}\right\rbrack$ .

【解析】解: (1) 由题意可知, 每吨垃圾分类处理的平均成本为月处理成本除以月处理量,
...
![17_144_683_603_308_0.jpg](images/17_144_683_603_308_0.jpg)

【难度】 $\star   \star   \star$

【答案】见解析

【解析】解: (1) 过点 $P$ 分别作 ${AB},{AD}$ 的垂线，垂足分别为 $E, F$ ，则 $\triangle PNF \sim \triangle MPE$ ，
