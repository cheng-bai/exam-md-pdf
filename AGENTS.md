...
- 单份试卷、小专题讲义、大型汇编材料必须分类归档，不把明显不是一类的资料放在同一个文件夹里。
...
- `handouts/<小专题中文名称>/`: 数学小专题讲义、教师版材料和专题习题册归档目录。
- `collections/<汇编类型>/<汇编成品中文名称>/`: 多校、多套、跨主题或大体量汇编成品归档目录。
- `资料来源/<来源名称>/`: 按资料提供者、机构或项目品牌保存 OCR、公式识别稿、历史版本和研发资料；承白及项目组制作资料统一归入 `资料来源/承白资料/`。
...
- 英文目录名可以保留，但每个根目录都应有中文 `README.md`，第一屏要写清“用途：……”，让用户一眼看懂该目录的作用。
- 目录说明应根据目录真实内容编写，不要只写抽象口号；例如 `inputs/` 是原始材料暂存，`outputs/` 是 OCR 中间结果暂存，`handouts/` 是专题讲义归档。
- GitHub 文件列表右侧显示的是该文件或目录最近一次相关提交的标题；如果多个目录被同一次提交修改，右侧会显示同一句提交标题。为了让目录说明更具体，目录说明类改动应按目录拆分提交。
- Markdown 引用图片时，必须使用仓库内相对路径，并提交对应图片文件；禁止留下本机绝对路径，如 `D:/...`。
