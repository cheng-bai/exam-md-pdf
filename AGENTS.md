...
- 云端归档目录名和目录内主文件基名必须一致，格式为：
  `学段_学校标准名_年级_学科_考试类型_版本类型_提交日期`
- 示例：
  `高中_上海市建平中学_高一_数学_2026春期末_解析版_20260625/高中_上海市建平中学_高一_数学_2026春期末_解析版_20260625.md`
  `高中_上海市建平中学_高一_数学_2026春期末_解析版_20260625/高中_上海市建平中学_高一_数学_2026春期末_解析版_20260625.pdf`
- `提交日期` 使用实际提交日的 `YYYYMMDD`，不是试卷日期；如果用户指定日期，以用户指定为准。
...
- 版本类型应反映交付状态，例如 `学生版`、`解析版`、`OCR审校学生版`、`学生版精校草稿`。未逐题终审时不要命名为 `正式版`。
- 目录内至少包含同名 `.md` 和 `.pdf` 两个顶层文件。若 Markdown 引用题图，必须同时提交必要的 `figures/`，保证 GitHub 上 Markdown 图片不失效。
...
- `exams/<学段_学校标准名_年级_学科_考试类型_版本类型_提交日期>/`: 推送到 GitHub 的最终试卷归档目录。
- `exams/<归档目录>/<同名>.md`: 最终 Markdown 主源稿。
- `exams/<归档目录>/<同名>.pdf`: 最终可打印 PDF。
- `exams/<归档目录>/figures/`: 最终 Markdown 引用的必要题图。
- 旧的 `exams/<slug>/source.md`、`outputs/`、MinerU 归档目录只作为本地工作现场或历史兼容目录；整理云端交付时不要主动纳入提交。
- `scripts/`: 项目自动化脚本。
- `examples/`: 示例材料。
