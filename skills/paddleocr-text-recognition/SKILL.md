...
description: >-
  Use when the requested output is exact line-level text or bounding boxes extracted from images,
  photos, scans, screenshots, or scanned PDFs. Returns machine-readable strings with optional bbox
  coordinates. Do not use as the main workflow for producing a finished exam Markdown/PDF or for
  reconstructing formulas, tables, or complex layouts.
...
> Project modification: the frontmatter description adds an exam-workflow routing exclusion. The upstream CLI instructions are unchanged.
