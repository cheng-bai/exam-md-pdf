...
- 单份试卷、小专题讲义、大型汇编材料分目录归档，不把明显不是一类的资料混在同一文件夹。
- 每份最终成品至少包含一个规范命名的 `.md` 和 `.pdf`。
- 必要题图放在同目录 `figures/`。
...
- 所有提交信息使用中文，便于在 GitHub 提交记录和文件列表中直接阅读。
- 提交标题要说明本次任务，提交正文要简要列出提交内容和校验结果。
- 英文目录名保留时，每个目录内的 `README.md` 必须用中文说明“用途”，让第一次打开仓库的用户能快速判断目录作用。
- 如果要让 GitHub 文件夹列表右侧显示不同的中文用途说明，应按文件夹分别提交对应 `README.md`。
- Markdown 中的图片引用必须改成仓库内相对路径，并把必要图片一起提交，避免他人在 GitHub 上打开 MD 时图片失效。
...
├── collections/               # 多校、多套或大体量汇编材料归档
├── 资料来源/                   # 按提供者或项目品牌归档来源资料和研发稿
...
- `exams/`：单份学校试卷、单次考试真题或同一试卷的不同版本。
- `handouts/`：围绕一个知识点、题型或小专题组织的讲义、教师版材料和专题习题册。
- `collections/`：多校、多套、跨主题或大体量汇编材料，例如多校试卷解析汇总、成套题集合集。
- `资料来源/`：按学科网、承白、教师或机构保存 OCR、公式识别稿、历史版本和研发资料，不作为正式成品目录。

不要把明显不是一类的资料放在同一个文件夹里；不确定时宁可新建更清晰的中文分类目录。
